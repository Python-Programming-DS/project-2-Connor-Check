...
class Game:
    ...
    '''
    So the minimax method works by simulating alternating turns across the entire board
    and continuing until it reaches an end state (either player wins or draws), it will then
    return back a score based on if the computer wins or not (1/-1/0). Thes

    '''
    def minimax(self, board_state, roboTurn):
        if self.checkWin("O"):
            return 1
        ...
        if roboTurn:
            ...
            return best_score
    '''
    The getBestMove method looks for any move that returns a non-negative score from the
    minimax method and returns that move. It doesn't take into account the depth, nor
    if one move results in more possible winning paths, it just picks the most recent good move.
    It's pretty simple this way but it works for Tic Tac Toe.
        ...
        for i in range(3):
            for j in range(3):
                if self.board.grid[i][j] == " ":
                    ...
                    score = self.minimax(self.board.grid, False)
                    self.board.grid[i][j] = " "
                    if score > best_score:
                        best_score = score
                        move = (i, j)
